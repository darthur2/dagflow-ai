...
from regressors import BernoulliRegressor, BetaRegressor, DiscreteUniformRegressor, ExponentialRegressor, GammaRegressor, LogNormalRegressor, NormalRegressor, UniformRegressor
...
def _sample_bernoulli_regression(x: np.ndarray, beta_0: float, beta_1_init: np.ndarray, c: float, seed: int = 0) -> np.ndarray:
    ...
    return stats.bernoulli.rvs(p, random_state=rng)


def _run_bernoulli_scenario(scenario: dict) -> dict[str, float]:
    x = _build_x_matrix(scenario["n"], scenario["p"], seed=scenario["seed_x"])
    ...
    target_mean = float(np.mean(samples))
    target_snr = float(np.var(np.mean(samples)) * 0 + scenario["target_snr"])
...
@pytest.mark.parametrize("scenario", BERNOULLI_SCENARIOS, ids=[scenario["name"] for scenario in BERNOULLI_SCENARIOS])
def test_bernoulli_regressor_synthetic_calibration(scenario):
    results = _run_bernoulli_scenario(scenario)
...
def _run_all_scenarios() -> int:
    failures = 0
    suites = [
        ("Normal", NORMAL_SCENARIOS, test_normal_regressor_synthetic_calibration),
        ("Exponential", EXPONENTIAL_SCENARIOS, test_exponential_regressor_synthetic_calibration),
        ("Gamma", GAMMA_SCENARIOS, test_gamma_regressor_synthetic_calibration),
        ("LogNormal", LOGNORMAL_SCENARIOS, test_lognormal_regressor_synthetic_calibration),
        ("Beta", BETA_SCENARIOS, test_beta_regressor_synthetic_calibration),
        ("Uniform", UNIFORM_SCENARIOS, test_uniform_regressor_synthetic_calibration),
        ("DiscreteUniform", DISCRETE_UNIFORM_SCENARIOS, test_discrete_uniform_regressor_synthetic_calibration),
        ("Bernoulli", BERNOULLI_SCENARIOS, test_bernoulli_regressor_synthetic_calibration),
    ]

    for suite_name, scenarios, test_fn in suites:
        for scenario in scenarios:
            try:
                test_fn(scenario)
            except AssertionError as exc:
                failures += 1
                print(f"FAILED {suite_name}[{scenario['name']}]: {exc}")
            except Exception as exc:
                failures += 1
                print(f"ERROR {suite_name}[{scenario['name']}]: {exc}")
...
if __name__ == "__main__":
    raise SystemExit(_run_all_scenarios())
