...
def test_gamma_regressor_distance_to_downtown_calibrates_and_samples():
    np.random.seed(0)
    ...
    response = Gamma(shape=3.0, rate=0.25, min=0.5, max=40.0)
    regressor = GammaRegressor(
        target_mean=response.target_mean(),
        target_variance=response.target_variance(),
        min=0.5,
...
def test_gamma_regressor_commute_time_calibrates_and_samples():
    np.random.seed(0)
    ...
    distance_to_downtown_mi = Gamma(shape=3.0, rate=0.25, min=0.5, max=40.0).sample(100)

    maple_grove = (municipality == "Maple Grove").astype(float)
    riverton = (municipality == "Riverton").astype(float)
    oak_park = (municipality == "Oak Park").astype(float)
    cedar_hills = (municipality == "Cedar Hills").astype(float)
    X = np.column_stack([distance_to_downtown_mi, maple_grove, riverton, oak_park, cedar_hills])

    response = Gamma(shape=4.0, rate=0.18, min=5.0, max=90.0)
    # Coefficients are intentionally conservative so the truncated gamma fit stays feasible.
    regressor = GammaRegressor(
        target_mean=response.target_mean(),
        target_variance=response.target_variance(),
        min=5.0,
...
def main() -> None:
    test_gamma_regressor_calibrates_and_samples()
    test_gamma_regressor_distance_to_downtown_calibrates_and_samples()
    test_gamma_regressor_commute_time_calibrates_and_samples()
